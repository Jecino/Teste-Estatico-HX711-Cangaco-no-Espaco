...
<body>
  <div class="panel">
    ...
    <canvas id="force_canvas" width="600" height="300"></canvas>
  </div>
  ...
  <script>
    ...
    chart.streamTo(document.getElementById("force_canvas"));
    var force_line = new TimeSeries();

    async function update(){
      try{
        const f_text = await (await fetch('/force_text')).text();
        const f_float = await (await fetch('/force_float')).text()
        force.textContent = f_text;

        setInterval(function() {
          force_line.append(Date.now(), f_float);
        }, 500);
...
void setup() {
  ...
  server.on("/force_text",[](){server.send(200,"text/plain",getForceString());});
  server.on("/force_float",[](){server.send(200,"text/plain",getForce());});
  server.begin();
}
